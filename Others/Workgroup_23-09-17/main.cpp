...
int fibReImp(int n) {
    ...
    if (n == 1 || n == 2) return 1;
        ...
        solvedFib.insert(pair<int, int>(n, (solvedFib.find(n - 1) != solvedFib.end() ? solvedFib.at(n - 1) : fibReImp(n - 1)) + (solvedFib.find(n - 2) != solvedFib.end() ? solvedFib.at(n - 2) : fibReImp(n - 2))));
